...
if IsServer() then
	...
	function pudge_meat_hook_lua:OnSpellStart()
		...
		if bloodMod ~= nil then
			if bloodMod:GetStackCount() > 0 then
				hook_speed = hook_speed + bloodMod:GetStackCount()
			end
		end
	...
	function pudge_meat_hook_lua:OnProjectileHit(hTarget, vLocation, proj, hook_damage)
		...
		if not IsCreepTCOTRPG(hTarget) then return false end
...
function modifier_meat_hook_bloodstained_lua:OnTooltip()
	local range = self:GetStackCount() * self:GetAbility():GetSpecialValueFor("hook_distance_per_stack")
	if range > 5000 then
		range = 5000
	end
	return range
