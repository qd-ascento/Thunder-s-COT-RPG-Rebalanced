...
function modifier_rebels_sword_passive:OnCreated( kv )
    ...
    if IsServer() then 
        ...
        self.disarmor_cd_separate_mnamme = "modifier_rebels_sword_disarmor_separate"
...
function modifier_rebels_sword_passive:OnAttackLanded( kv )
    ...
    local total_stack_count = stack_count + (self.disarmor*armor )
    ...
    if not target:HasModifier(self.disarmor_cd_separate_mnamme) then
        target:AddNewModifier(target, ability, self.disarmor_cd_separate_mnamme, { duration = self.duration } )
    else
        target:FindModifierByName(self.disarmor_cd_separate_mnamme):ForceRefresh()
    end
